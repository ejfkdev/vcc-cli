...
从 [Releases](https://github.com/ejfkdev/vcc-cli/releases/latest) 下载适合你平台的最新二进制文件。
...
VCC 维护一个**资源注册表**（`~/.vcc/registry/`），每个资源对应一个 TOML 文件。执行 `apply` 时，VCC 读取注册表并以正确的格式（JSON/TOML/YAML）写入各工具的配置目录。
